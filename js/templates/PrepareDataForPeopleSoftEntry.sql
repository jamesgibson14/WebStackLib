...
FROM 
	...
	FROM (
			...
			endscrap = (
				ROUND(ItemsOnMachineCount.PaperScrap*
					...
					* Items.[Feedup]) / --Feet of paper used
					(CASE WHEN Machines.WorkCenter_ID=52202 THEN 9000 ELSE 22000 END) --10000 is a guess of the avg feet of paper on a roll for each roll change.
					)--run Ceiling on number of rolls to always give an integer
				,0)
			),
		...
		WHERE     
			...
			AND	ProductionDataDetails.PID IN ('PID1623880', 'PID162388')
